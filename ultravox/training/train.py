...
@record
def main() -> None:
    # 1. Disable parallelism to avoid deadlocks in DataLoader, apparently
    ...
    #WORLD_SIZE parameter refers to the total number of processes or devices (e.g., GPUs) participating in the training.
    #DistributedDataParallel ,  WORLD_SIZE would be 32 (4 nodes × 8 GPUs per node), LOCAL_RANK would range from 0 to 7, representing the 8 GPUs within each node.
    world_size = int(os.environ.get("WORLD_SIZE", 1)) #GPU
    local_rank = int(os.environ.get("LOCAL_RANK", 0)) #unique id for current process
    is_master = local_rank == 0


    #-backend="nccl": The backend parameter specifies the communication backend to be used for the distributed training. In this case, the "nccl" (NVIDIA Collective Communications Library) backend is selected.
    # The "nccl" backend is a popular choice for distributed training on NVIDIA GPUs, as it provides high-performance, GPU-optimized communication primitives. Other available backends include "gloo" (a CPU-based communication library) and "mpi" (for use with the Message Passing Interface).
    if world_size > 1:
        # DDP: initialize the process group across multiple GPUs
        torch.distributed.init_process_group(backend="nccl") # on Nvida GPU, communication backend with GPUs

    # 2. DDP blows up logging, so this is an attempt to suppress it to only logs from the master process
    ...
    # 3. Instantiate the model and processor
    ...
    # 4. Since the model downloads the language model and audio encoder weights, we want one process to finish up
    ...
    dtype = getattr(torch, args.data_type) #torch.bfloat16 if args.data_type == "bfloat16" else torch.float32
